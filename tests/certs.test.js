...
describe('Certs', () => {
    ...
    const valid = "10h";
    const cert_serial = ["22:16:df:60:c2:99:bc:c4:9b:1d:fd:71:5e:e9:07:d9:1b:3c:85:1d"];
    ...
    test('Issue should handle a conflict error', () => {
        const errorResponse = {
            response: {
                status: 500,
            },
        ...
        return sdk.certs.Issue(thing_id, valid, token).then(result => {
            ...
            expect(result).toBe("Unexpected server-side error occurred.");
        });
    });
    ...
    test('ViewByThing should handle a conflict error', () => {
        const errorResponse = {
            response: {
                status: 500,
            },
    ...
    test('ViewBySerial should handle a conflict error', () => {
        const errorResponse = {
            response: {
                status: 500,
            },
    ...
    test('Revoke should handle a conflict error', () => {
        const errorResponse = {
            response: {
                status: 500,
            },
